...
**Production infrastructure:** EC2 t3.micro running Docker + Nginx (HTTP → Spring Boot), fronted by its own CloudFront distribution for free HTTPS. The React build is served from S3 through a second, separate CloudFront distribution, whose origin is the S3 *website* endpoint (not the S3 REST API) — so missing objects return a plain 404 from S3, not a CloudFront-native error. Because this is a single-page app using client-side routing (React Router), that distribution has a **custom error response** (404 → `/index.html`, HTTP 200) so that a direct browser navigation or refresh on any route (e.g. `/group-stage`, `/squad`) falls through to `index.html` and lets React Router handle it, instead of 404ing at the CDN/S3 layer. GitHub Actions builds the Docker image, pushes to Amazon ECR, deploys via SSH to EC2, and uploads the React build to S3. The backend and frontend deploy jobs each run independently and are gated by [`dorny/paths-filter`](https://github.com/dorny/paths-filter) — a push to `main` only redeploys the component(s) whose files actually changed (`backend/**` or `frontend/**`, excluding `README.md` changes, which never trigger a deploy on their own).
